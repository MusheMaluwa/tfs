...
router.get('/', requireAuth, async (req, res) => {
  ...
  const manifests = await db.all(sql, params);
  ...
  const withAssets = await Promise.all(manifests.map(async (m) => ({
    ...m,
    assets: await db.all(`SELECT asset_id, expected, scanned, scanned_at FROM manifest_assets WHERE manifest_id = ?`, [m.id]),
  })));
  res.json(withAssets);
});

router.get('/:id', requireAuth, async (req, res) => {
  const manifest = await db.get(`SELECT * FROM manifests WHERE id = ?`, [req.params.id]);
  if (!manifest) return res.status(404).json({ error: 'not found' });
  const assets = await db.all(`SELECT asset_id, expected, scanned, scanned_at FROM manifest_assets WHERE manifest_id = ?`, [req.params.id]);
